...
function recommendMovies({ moviesData, age, gender, preferredGenres }) {
  ...
  const scored = moviesData.map((movie) => {
    ...
    if (preferredGenres.length > 0 && movie.genre) {
    ...
    switch (step) {
      case STEPS.GREETING: {
        if (lower.includes('bắt đầu') || lower.includes('tư vấn') || lower.includes('ok') || lower.includes('được')) {
          ...
          simulateTyping(() => {
            const nowShowing = moviesData.filter((m) => m.status === 'now_showing');
            addBotMessage({
      ...
      case STEPS.ASK_GENRE: {
        ...
        simulateTyping(() => {
          ...
          const recommended = recommendMovies({ moviesData, age: info.age, gender: info.gender, preferredGenres: genres });
