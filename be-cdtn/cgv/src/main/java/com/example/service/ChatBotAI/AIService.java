...
public class AIService {
    ...
    public String askGemini(String prompt) {
        try {
            ...
            contextBuilder.append("Dưới đây là thông tin hiện tại từ hệ thống để bạn tham khảo khi trả lời khách hàng:\n");
            ...
            for (Slots slot : slots) {
                if (!Boolean.TRUE.equals(slot.getIsDeleted()) && slot.getMovies() != null && slot.getRooms() != null && slot.getRooms().getCinemas() != null) {
                    contextBuilder.append(String.format("  + Phim: %s | Rạp: %s - %s | Lịch chiếu: %s\n", 
                        slot.getMovies().getTitle(), slot.getRooms().getCinemas().getCinemaName(), slot.getRooms().getRoomName(), slot.getShowTime()));
                }
            }
            ...
            Content systemInstruction = Content.fromParts(
                    Part.fromText(systemInstructionText));
